...
public class GetPostsByUserQueryTests
{
	...
	public async Task Handle_TargetUserNotFound_ShouldThrowEntityNotFoundException()
	{
		...
		await Assert.ThrowsAsync<EntityNotFoundException>(() => handler.Handle(
			new GetPostsByUserQuery { UserId = targetUserId },
	...
	public async Task Handle_WhenBlockedRelationshipExists_ShouldReturnEmptyPage()
	{
		...
		var result = await handler.Handle(
			new GetPostsByUserQuery { UserId = targetUserId, PageNumber = 2, PageSize = 5 },
			CancellationToken.None);
		...
		_postRepositoryMock.Verify(
			x => x.GetVisibleByUserAsync(It.IsAny<Guid>(), It.IsAny<RequestParameter>()),
			Times.Never);
	}

	[Fact]
	public async Task Handle_ShouldReturnVisiblePostsAndSetIsUpvoted()
	{
		var currentUserId = Guid.NewGuid();
		var targetUserId = Guid.NewGuid();
		...
		{
			new() { UserId = currentUserId, PostId = visiblePostId }
		}).Object);

		_postRepositoryMock
			.Setup(x => x.GetVisibleByUserAsync(
				targetUserId,
				It.Is<RequestParameter>(p => p.PageNumber == 1 && p.PageSize == 20)))
			.ReturnsAsync(new PagedResponse<Post>(
				new List<Post>
				{
					new()
					{
						...
						IsVisible = true,
						User = new User { Id = targetUserId, Username = "target", Email = "target@example.com" }
					}
				},
				1,
				20,
		...
		result.Data[0].Id.Should().Be(visiblePostId);
		result.Data[0].Content.Should().Be("Visible post");
		result.Data[0].IsVisible.Should().BeTrue();
		result.Data[0].IsUpvoted.Should().BeTrue();
	}

	private GetPostsByUserQueryHandler CreateHandler()
	{
		return new GetPostsByUserQueryHandler(
